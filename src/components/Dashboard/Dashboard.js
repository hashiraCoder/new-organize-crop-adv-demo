...
import React, { useState } from 'react';
import Sidebar from './Sidebar';
import Weather from './Weather'; // Corrected path
import DiseaseDetection from '../DiseaseDetection';
import Chat from '../Chat';
import CropRecommendation from '../CropRecommendation';
...
const Dashboard = ({ language, setLanguage, handleLogout }) => {
  const [activePage, setActivePage] = useState('Dashboard'); // Start on Dashboard
  ...
  const renderContent = () => {
    switch (activePage) {
      ...
      case 'Crop Recommendation':
        return <CropRecommendation language={language} />; 
      default:
        // Pass language and setActivePage to the main content
        return <DashboardContent setActivePage={setActivePage} language={language} />; 
    }
  };
...
// Main content for the dashboard homepage
const DashboardContent = ({ setActivePage, language }) => {
  return (
    <div className="animate-fade-in-down">
      <h1 className="text-3xl md:text-4xl font-bold text-gray-800">{strings[language].dashboardTitle}</h1>
      <p className="text-gray-600 mb-8">{strings[language].dashboardSubtitle}</p>
      
      {/* Quick Actions section */}
      <h2 className="text-2xl font-bold text-gray-800 mb-4">{strings[language].quickActions}</h2>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        <QuickActionButton 
            onClick={() => setActivePage('Detect Disease')} 
            color="red" 
            label={strings[language].diseaseButton} 
            icon="🐞"
        />
        <QuickActionButton 
            onClick={() => setActivePage('Weather')} 
            color="blue" 
            label={strings[language].weatherButton}
            icon="🌦️" 
        />
        <QuickActionButton 
            onClick={() => setActivePage('Market Prices')} 
            color="yellow" 
            label={strings[language].marketPricesButton}
            icon="💲" 
        />
        <QuickActionButton 
            onClick={() => setActivePage('AI Assistant')} 
            color="purple" 
            label={strings[language].chatbotButton}
            icon="🤖" 
        />
      </div>
    </div>
...
// Reusable button component for the dashboard
const QuickActionButton = ({ onClick, color, label, icon }) => {
  const colors = {
    red: 'bg-red-500 hover:bg-red-600',
  ...
  return (
    <button 
      onClick={onClick} 
      className={`flex flex-col items-center justify-center text-white font-bold py-6 px-4 rounded-lg shadow-md transition-transform transform hover:scale-105 ${colors[color]}`}
    >
